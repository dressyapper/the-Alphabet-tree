...
function layerDataReset(layer, keep = []) {
	...
	if (tmp[layer].onPrestigeAfter) {
		tmp[layer].onPrestigeAfter()
	}
}
...
function gameLoop(diff) {
	...
	for (let x = 0; x <= maxRow; x++){
		for (item in TREE_LAYERS[x]) {
			...
			if (tmp[layer].passiveGeneration && !player.paused) generatePoints(layer, diff*tmp[layer].passiveGeneration);
